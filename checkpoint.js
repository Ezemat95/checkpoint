...
function cuantosOnline(usuarios) {
  ...
  // Tu código aca:
  // usuarios es la caja de bananas y 'banana' son las bananas dentro de la caja
...
function palabraMasLarga(array) {
  ...
  // Tu código aca:
  //
 var palabraultralarga = "";
 var almacen = 0;
for( var i=0; i < array.length; i++ ){
  var arrayaux = array[i].split(' ');
    for( var j=0; j < arrayaux.length; j++ ) {
        if(arrayaux[j].length > almacen ){
         almacen = arrayaux[j].length 
         palabraultralarga = arrayaux[j]
        }
      }

}
return palabraultralarga;
